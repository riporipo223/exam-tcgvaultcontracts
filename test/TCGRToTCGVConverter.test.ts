...
async function deployFixture() {
  ...
  return { owner, minter, user, tcgr, mockTcgv, converter };
}
...
describe("TCGRToTCGVConverter", function () {
  ...
  it("convert burns TCGR and sends TCGV at 1:1 ratio", async function () {
    const { tcgr, mockTcgv, converter, minter, user } = await networkHelpers.loadFixture(deployFixture);
    const amount = parseEther("500");
    await tcgr.write.mintReferral([user.account.address, amount], { account: minter.account });
  ...
  it("convert with ratio 0.5: 100 TCGR gives 50 TCGV", async function () {
    const { tcgr, mockTcgv, converter, owner, minter, user } = await networkHelpers.loadFixture(deployFixture);
    ...
    await tcgr.write.mintReferral([user.account.address, parseEther("100")], { account: minter.account });
    const userTcgvBefore = await mockTcgv.read.balanceOf([user.account.address]);
  ...
  it("convert reverts when insufficient TCGV reserve", async function () {
    const { tcgr, mockTcgv, converter, minter, user } = await networkHelpers.loadFixture(deployFixture);
    await tcgr.write.mintReferral([user.account.address, parseEther("200000")], { account: minter.account });
    await viem.assertions.revertWithCustomError(
  ...
  it("convert reverts ZeroAmount when tcgvOut is zero (tiny amount)", async function () {
    ...
    await tcgr.write.mintReferral([user.account.address, 1n], { account: minter.account });
    await viem.assertions.revertWithCustomError(
